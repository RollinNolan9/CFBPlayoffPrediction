...
```{r}
cfb_pbp23_test <- load_cfb_pbp(2023:2024)
cfb_pbp19_22_train <- load_cfb_pbp(2014:2022)
cfb_prediction2024 <- load_cfb_pbp(2025)
...
clean_team_names <- function(data) {
  data <- data %>%
    ...
    filter(home != "Kennesaw State" & away != "Kennesaw State")
...
sp_ratings_all_years <- lapply(2014:2025, get_sp_ratings) %>%
  bind_rows()
...
pff_team_data <- pff_team_data %>% select(-Coverage)

coach_data <- read.csv("coach_ratings (1).csv", header = TRUE, stringsAsFactors = FALSE)

injury_data <- read.csv("final_for_sure_1.csv", header = TRUE, stringsAsFactors = FALSE)
...
season_level_data_train <- season_level_data_train %>%
  ...
  rename(home_pf = PF,
         ...
         home_off = Offense,
         home_pass = Passing, 
         home_pblk = Pass_Block,
         home_recv = Receiving,
         home_run = Running,
         home_rblk = Run_Block, 
         home_def = Defense,
         home_rdef = Run_Defense, 
         home_tack = Tackling,
         home_prsh = Pass_Rush,
         )
...
season_level_data_train <- season_level_data_train %>%
  ...
  rename(away_pf = PF,
         ...
         away_off = Offense,
         away_pass = Passing, 
         away_pblk = Pass_Block,
         away_recv = Receiving,
         away_run = Running,
         away_rblk = Run_Block, 
         away_def = Defense,
         away_rdef = Run_Defense, 
         away_tack = Tackling,
         away_prsh = Pass_Rush
         )
...
train_dat <- season_level_data_train[, c(6:7, 26:33, 36:44, 49:59, 64:74)]  
...
test_dat <- season_level_data_test[, c(6:7, 26:33, 36:44,49:59, 64:74)]  
...
predict_dat <- season_level_data_predict[, c(6:9, 26:33, 36:44, 49:59, 64:74)]
...
predict_winner <- function(home_team, away_team, model, data) {
  ...
  new_row <- data.frame(
    ...
    home_over = home_game$home_over, away_over = away_game$away_over,
    home_off = home_game$home_off, away_off = away_game$away_off,
    home_pass = home_game$home_pass, away_pass = away_game$away_pass,
    home_pblk = home_game$home_pblk, away_pblk = away_game$away_pblk,
    home_recv = home_game$home_recv, away_recv = away_game$away_recv,
    home_run = home_game$home_run, away_run = away_game$away_run,
    home_rblk = home_game$home_rblk, away_rblk = away_game$away_rblk,
    home_def = home_game$home_def, away_def = away_game$away_def,
    home_rdef = home_game$home_rdef, away_rdef = away_game$away_rdef,
    home_tack = home_game$home_tack, away_tack = away_game$away_tack,
    home_prsh = home_game$home_prsh, away_prsh = away_game$away_prsh
