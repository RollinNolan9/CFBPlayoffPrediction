...
The production core is weighted ridge regression. Numeric inputs are median-imputed,
centered, and scaled using training-only values. Ridge lambda is selected through
...
Important current limitation: the external historical preseason inputs have not yet
been populated or evaluated. The committed source CSVs are contracts with headers,
not evidence that those features are active.

Research during the session confirmed that CFBD provides historical 2021-2025:

- returning PPA and returning usage
- Week 1 AP and Coaches poll points
- 247Sports team-talent composites

These will be backfilled and evaluated as challengers before 2026 data is appended.
Raw preseason rank will not enter the production core. Planned derived values include
retained quality, replacement capacity, and poll hype gap.

## 11. Run the August 29, 2026 Dry Run

`cfb_v2/dry_run_aug29_2026.R` creates an isolated eight-game Week 1 exercise using a
July 15 DraftKings snapshot, current coach mappings, and only information available
...
- FanDuel was unavailable in the captured CFBD response, so the snapshot uses
  DraftKings only.
- North Dakota State and Sacramento State are flagged as 2026 FCS-to-FBS transitions.
- Tim Polasek, Alonzo Carter, and Tavita Pritchard currently lack modeled prior head
  coach history and receive neutral coach contributions.

The transition-game projections are diagnostic only. Missing FBS history created
extreme success and turnover contributions that the current uncertainty estimate does
not cover.

Regenerate the dry run with:

```powershell
& 'C:\Program Files\R\R-4.2.2\bin\Rscript.exe' .\cfb_v2\dry_run_aug29_2026.R
```

Generated reports, predictions, and charts are written beneath
`cfb_v2/output/2026/august_29_dry_run` and are intentionally ignored by Git.
