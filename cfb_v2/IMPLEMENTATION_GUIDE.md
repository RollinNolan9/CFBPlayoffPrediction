...
- a requested article-pick option for games that need a published side

An article pick chooses the side favored by the model margin edge but records
`article_pick`; it supplies the requested side without pretending the validated
...
- Slate: article picks, validated best bets, review flags, and the filterable
  full card.
...
Do not treat PFF, weather, conference labels, or raw poll rank as production features
without separate challenger evidence and rolling validation. Portal inputs remain
preseason-only, season-normalized challenger evidence and reach zero from Week 5.
