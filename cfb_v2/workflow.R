...
write_v2_backtest_report <- function(summary, predictions, coach_comparison, path,
  ...
  report <- c(
    ...
    "",
    "Rolling folds train only on earlier seasons. The production ridge core uses the full",
    "preseason model through Week 4 while its roster inputs fade once at the feature layer",
    "by 100/60/30/10%. It switches exactly to the foundation from Week 5. Team-specific HFA",
    "and the residual",
...
write_preseason_challenger_report <- function(predictions, output_dir,
  ...
  header <- c(
    ...
    paste0("Generated: ", format(Sys.time(), tz = "UTC", usetz = TRUE)),
    "",
    paste0("The ridge core is the production model: the full preseason challenger leads ",
           "through Week 4 while returning production, retained quality, 247 talent ",
           "percentile, replacement capacity, and incoming-transfer production fade once ",
           "at the feature layer by 100/60/30/10%. Week 5 switches exactly to foundation. ",
    ...
    paste0("`preseason_ablation_challenger` is the pure foundation control, while ",
           "`preseason_full_challenger` verifies the full early-season model without ",
           "the production Week 5 foundation switch. ",
  ...
  if (!is.null(coefficients) && nrow(coefficients)) {
    report <- c(
      ...
      paste0("Descriptive full-sample ridge coefficients at each variant's selected ",
             ...
             "table above. `ridge_core` shows the full phase-faded preseason effect."),
      "",
